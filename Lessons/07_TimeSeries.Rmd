...
start with line plot (observe trend or seasonal component)
composing the series (divide into three: seasonal, trend, reminder) <--sdl \
bars -- sense of relativeness 
seasonality affect discharge  
...
### Monotonic trend analysis <-> stochastic trend analysis 

1. linear regression 
2. mann-kendall: no seasonality non-parametic missing data allowed ManKandall()
3. Seasonal Mann- Kendall: seasonality non-parametic SeasonalMannKendall()
4.
...
Note that wind speed has a s
easonal cycle. We might be interested in knowing how (if) speed has changed over the course of measurement while incorporating the seasonal component. In this case, we will use a Seasonal Mann-Kendall test to figure out whether a monotonic trend exists.
