...
ggplot (Rainfall_BR_processed, aes( x= Date, y =Rainfall))+
  geom_line()+
  geom_smooth()
#NO Trend in rainfall data, but YES for seasonal components

ggplot(Temp_BR_processed, aes(x= Date, y = Temperature_C))+
  geom_line()+
  geom_smooth(method= "lm")

#Yes there are both trend and seasonality 
...
BRA_rainfall_ts <- ts(BRA_complete$Rainfall, start = c(f_year,f_month), frequency = 12)
...
BRA_complete_Decomposed <- stl(BRA_complete_ts, s.window = "periodic")
plot(BRA_complete_Decomposed)
...
BRA_complete_Components <- as.data.frame(BRA_complete_Decomposed$time.series[,1:3])

BRA_complete_Components <- mutate(BRA_complete_Components, o
                                  )
...
ggplot(BRA_complete_Components) +
  geom_line(aes(y =Rainfall, x = Date), size = 0.25) +
  geom_line(aes(y= Temperature_C, x= Date), color = "red")+
  geom_hline(yintercept =0, lty=2)
