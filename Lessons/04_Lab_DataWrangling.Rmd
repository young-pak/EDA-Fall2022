...
# filter/ arrange/ select/ mutate/ pipes/ lubricate (seperate from tidyverse works with dates)
...
#install.packages('formatR') // want to comment out install becasue it will not let knitting work 
#double colon, 
...
# dplyr filtering
#seperating factor of what factor they are 
...
NTL.phys.data.ex3 <- 
  ...
  mutate (month= month(sampledate)) %>% # create a new column 
  filter (month == 7 ) 
...
EX5 <- 
  NTL.phys.data %>% 
  pivot_longer(
    irradianceWater:irradianceDeck, #columns that i want to combine 
    names_to ="irradiance",
    values_to = "radiation"
  )
