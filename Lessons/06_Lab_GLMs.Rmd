...
Boxplot <- ggplot(EPAair, aes(x=as.factor(Year), y = Ozone))+
  geom_boxplot()+
...
summary(regression)
##13% of ozone of explained by pm2.5
...
TPAIC <- lm(data = EPAair, meanLat ~ Ozone)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
TPmodel <- lm(data = EPAair, meanLat ~ Ozone)
summary(TPmodel)

#AIC is like a scale -- the lower ACI the better
...
OzoneBestReg <-lm(data = corr, meanLat~ Ozone)
summary(OzoneBestReg)
...
# Graph the results
Litter.Totals.plot <- ggplot(Litter.Totals.2, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
